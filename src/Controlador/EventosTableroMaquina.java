...
// MOV MAQUINA
               int fil=0;
                ...
                while(camp.get_Ficha(fil, col)==null ||  camp.get_Ficha(fil, col).getBando()==0 ){
                fil=(int) (Math.random() * 8);
                col=(int) (Math.random() * 8);
                if(camp.get_Ficha(fil, col)!=null && camp.get_Ficha(fil, col).getBando()==1){
               System.out.println("Ficha "+ fil+" " +col);
              Ficha[][] temptab=this.camp.get_Ficha();
              int i=0;
              while(temptab[fil][col].evaluarMovimiento(temptab, movfil, movcol, muertas)==false && i!=1000){
               temptab=this.camp.get_Ficha();
              if(temptab[fil][col].evaluarMovimiento(temptab, movfil, movcol, muertas)==true){
                  System.out.println("Se puede mover a "+movfil+" "+movcol);
  ...
  if(temptab2[fil][col].evaluarMovimiento(temptab2, movfil, movcol, muertas)){
  this.camp.get_Ficha(fil, col).mover(this.camp.get_Ficha(), movfil, movcol, muertas);
  this.tab.get_LblImgFicha()[movfil][movcol].setIcon(this.tab.get_LblImgFicha()[fil][col].getIcon());
